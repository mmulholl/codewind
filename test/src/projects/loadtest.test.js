...
describe.skip('Load Runner Tests', function() {
    describe('loadtest/config Tests', function() {
        ...
        const templateID = 'nodeExpressTemplate';
        const configOptions = {
            path: '/',
        ...
        before(async function() {
            ...
            projectID = await projectService.bindNonBuiltProjectFromTemplate(projectName, workspace_location, templateID);
        });
        ...
        describe('POST loadtest/config', function() {
            describe('Valid Input', function() {
                describe('Complete valid options', () => {
                    it('should succeed (with status 200) to POST/runLoad/config to the load-test/config.json', async function() {
                        this.timeout(testTimeout.short);
                        const res = await writeToLoadTestConfig(projectID, configOptions);
                        res.should.have.status(200);
                    });

                    it('should succeed (with status 200) to POST/runLoad/config when query parameters are included', async function() {
                        this.timeout(testTimeout.short);
                    ...
                    it('should succeed (with status 200) to GET/runLoad/config with the correct decoded options', async function() {
                        this.timeout(testTimeout.short);
                    // Query parameters are processed into a query string and not saved
                        delete configOptions.query;
                        expectedSavedConfig = modifyLoadTestConfigOptions(configOptions, { path: '/?a=1&b=2' });
                ...
                describe('Additional invalid field', () => {
                    it('should succeed (with status 200) to POST/runLoad/config when non-config fields are included', async function() {
                        this.timeout(testTimeout.short);
                        configOptions.query = { a :'1', b :'2' };
                    ...
                    it('should succeed (with status 200) to GET/runLoad/config and not return these non-config fields in runLoad/config.json', async function() {
                        this.timeout(testTimeout.short);
                        const res = await readLoadTestConfig(projectID);
                        res.should.have.status(200);
                        res.body.should.deep.equal(expectedSavedConfig);
        ...
        describe('GET loadtest/config', () => {
            describe('Valid Input', () => {
                it('should succeed (with status 200) to GET/loadtest/config from the load-test/config.json', async function() {
                    this.timeout(testTimeout.short);
                    const res = await readLoadTestConfig(projectID);
                    res.should.have.status(200);
                    res.body.should.deep.equal(expectedSavedConfig);
    ...
    // Leave skipped for now as issue with projects not running before, load-run starts. TODDO: Investigate and unskip
    describe('runLoad Tests', function() {
        const projectName = `loadrunnertest${Date.now()}`;
        const templateID = 'nodeExpressTemplate';
        let projectID, workspace_location, projectPath;
...
function modifyLoadTestConfigOptions(options, newOptions) {
