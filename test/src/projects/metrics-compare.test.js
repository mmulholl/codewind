...
describe.skip('Metrics Comparison tests', function() {
    let projectID;
    let projectLoadTestPath;

    before('Create project and get loadTestPath', async function() {
        this.timeout(testTimeout.med);
        projectID = await projectService.createProjectAndAwaitID({ name: `metricscompare${Date.now()}` });
    ...
    describe('GET/projects/{id}/compare', function() {
        it('should return the correct comparison data', async function() {
        ...
        it('should fail with status 404 if the given project does not exist', async function() {
            this.timeout(testTimeout.short);
            const res = await getProjectComparisonData('blahblah');
            res.should.have.status(404);
        });

        it('should, if CPU data is deleted, find CPU \'Data unavailable\', but still find valid data for other types', async function() {
            this.timeout(testTimeout.med);
        ...
        it('should fail (with status 422) if there are fewer than 2 snapshots available', async function() {
            this.timeout(testTimeout.short);
