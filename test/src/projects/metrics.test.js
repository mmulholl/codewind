...
const { ADMIN_COOKIE, USING_K8S, METRIC_TYPES, MOCK_LOADTEST_DIR, testTimeout } = config;

chai.should();

// Bind/Unbind Changes - This test uses function we will no longer have.
describe.skip('Project Metrics tests', function() {
    let projectID;
    ...
    before('Create project and save the projec\'ts loadTestPath', async function() {
        ...
        projectID = await projectService.createProjectAndAwaitID({ name: `projectmetricstest${Date.now()}` });
