...
async function getAppContainers() {
  ...
  const mcContainers = containers.filter(container => (
    container.NetworkSettings.Networks.microclimate_network
  ));
...
function summariseContainerInfo(mcContainers) {
  return mcContainers.map(container => {
    ...
    containerInfo.ip = Networks.microclimate_network.IPAddress; // non-mc containers won't have this, so this will error
    if (Ports) {
