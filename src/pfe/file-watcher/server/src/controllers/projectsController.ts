...
export async function createProject(req: ICreateProjectParams): Promise<ICreateProjectSuccess | ICreateProjectFailure> {
    ...
    const allProjectTypes = projectExtensions.getAllProjectTypes();

    if (!allProjectTypes.includes(projectType)) {
        const msg = "The project type " + projectType + " is not supported";
        logger.logProjectError(msg, projectID, projectName);
        return {  "statusCode": 404, "error": {"msg": msg }};
    }

    const selectedProjectHandler = projectExtensions.getProjectHandler(projectType);
    ...
    if (startMode) {
        ...
        if (settings && settings.internalAppPort) {
            ...
            projectInfo.appPorts.push(selectedProjectHandler.getDefaultAppPort());
        }
...
function killProcesses(psOutput: string, projectID: string, projectLocation: string): void {
    ...
    // 76 root       0:00 {project-watcher} /bin/bash /file-watcher/scripts/project-watcher.sh /microclimate-workspace/springy /Users/rajiv/git/microclimate/microclimate-workspace &
    ...
    // 105 root       0:00 inotifywait --format %T:%f --timefmt %s -e CREATE -e CLOSE_WRITE -e MODIFY -e DELETE -r -m /microclimate-workspace/springy/src /microclimate-workspace/springy/pom.xml /microclim
...
export async function projectDeletion(projectID: string): Promise<number> {
    ...
    try {
        ...
        if (projectInfo.projectType) {
            const selectedProjectHandler = projectExtensions.getProjectHandler(projectInfo.projectType);
