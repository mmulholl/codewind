 ...
 * @returns Promise<ProcessResult>
 */
export async function installChart(projectID: string, deploymentName: string, chartLocation: string): Promise<ProcessResult> {
    ...
    const installDeployment: string[] = ["upgrade", "--install", deploymentName, "--recreate-pods", "--values=/file-watcher/scripts/override-values-icp.yaml", "--set", "image.repository=" + dockerRegistry + "/" + deploymentName, "--set", "microclimate.release.name=" + microclimateReleaseName, chartLocation];
    let response: ProcessResult;
