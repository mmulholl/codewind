...
MC_WS_DIR=~/microclimate-workspace
...
if isMicroclimateRunningOnWindows; then
   ...
   MC_CLI_DIR=$MC_RELEASE_DIR/cli
   MC_WS_DIR=$MC_RELEASE_DIR/microclimate-workspace
fi
...
if [ $MC_UNINSTALL_TYPE == "local" ]; then
   ...
   echo -e "${BLUE}Removing microclimate network... ${RESET}"
   NUMBER_OF_MC_NETWORK=$(docker network ls | grep microclimate_network | wc -l)

   if [ $NUMBER_OF_MC_NETWORK -gt 0 ]; then
      docker network rm microclimate_network
      if [ $? -eq 0 ]; then
         echo -e "${GREEN}Successfully removed microclimate network. ${RESET}\n"
      else
         echo -e "${RED}Failed to remove microclimate network. ${RESET}\n"
         exit 1
      fi
   else
      echo -e "${GREEN}There is no microclimate network to remove. ${RESET}\n"
   fi

   echo -e "${BLUE}Removing default network... ${RESET}"
   NUMBER_OF_DEFAULT_NETWORK=$(docker network ls | grep microclimate_default | wc -l)

   if [ $NUMBER_OF_DEFAULT_NETWORK -gt 0 ]; then
      docker network rm microclimate_default
      if [ $? -eq 0 ]; then
   ...
   # Remove microclimate workspace
   echo -e "${BLUE}Removing microclimate workspace... ${RESET}"
   if [ -d $MC_WS_DIR ]; then
      rm -rf $MC_WS_DIR
      if [ $? -eq 0 ]; then
         echo -e "${GREEN}Successfully removed microclimate workspace. ${RESET}\n"
      else
         echo -e "${RED}Failed to remove microclimate workspace. ${RESET}\n"
         exit 1
      fi
   else
      echo -e "${GREEN}There is no microclimate workspace to remove. ${RESET}\n"
   fi
