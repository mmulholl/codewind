...
if [[ -n $TRAVIS_PULL_REQUEST ]]; then
   ...
   MC_DIR=$PWD
   WORKSPACE_PROP=-Dmicroclimate.workspace=$MC_DIR/microclimate-workspace
...
# Since filewatcher has the limitation of building projects under microclimate-workspace directory
# Need to copy all existing projects to microclimate-workspace directory for testing
# Once filewatcher resolves the limitation, the following code of copying existing projects can be removed
MICROCLIMATE_WORKSPACE=$MC_DIR/microclimate-workspace
PROJECTS_RESOURCES_DIR=$TEST_DIR/resources
echo -e "${BLUE}Copying all existing projects to microclimate-workspace directory. ${RESET}"
cp -r $PROJECTS_RESOURCES_DIR/* $MICROCLIMATE_WORKSPACE
if [ $? -eq 0 ]; then
   echo -e "${GREEN}Successfully copied all existing projects to microclimate-workspace directory. ${RESET}\n"
else
   echo -e "${RED}Failed to copy all existing projects to microclimate-workspace directory. ${RESET}\n"
   exit 1
fi
...
if isMicroclimateRunningOnWindows; then
   ...
   WORKSPACE_PROP=-Dmicroclimate.workspace=/c/microclimate-release/microclimate-workspace
fi
